...
当前版本是 **v0.4.1**：除进程发现、完整属性预览、兼容扫描、全量筛选和本地导出外，已接入由 screenshare.cn 鉴权、KOOK 帖子频道保存的客户端加密归档与恢复链路，并会自动配对恢复凭证与归档编号。
...
- 管理人员可在后台按用户、状态、进程或归档编号查找记录，并为已封存归档生成 1 小时恢复凭证；后台可一键复制包含凭证与归档编号的完整恢复信息。
- Dumper 可从完整恢复信息自动填入归档编号；即使手工输入了错误编号，也会在下载前按有效恢复凭证自动纠正，不会读取该凭证授权范围以外的归档。
...
7. 恢复时由后台打开目标记录并生成恢复凭证，点击“一键复制恢复信息（推荐）”，将整份内容粘贴到 Dumper 的“短期凭证”框；归档 UUID 会自动填入。随后点击“恢复归档”并选择目标文件。
